  ...
  return (
    <>
      ...
      <Layout title="Rules" descr="рисунок" colorBg="white" urlBg={LayoutImage} >
        ...
        If the rank of the opponent's card is higher than the player's card, the player's card will be captured and turned into the opponent's color. If the player's rank is higher,
      ...
      <Layout title="Зима" descr="Какая чудная погода" colorBg="green" >
        <div className="flex">
          {
            POKEMONS.map(item => <PokemonCard key={item.id} id={item.id} values={item.values} img={item.img} name={item.name} type={item.type} />)
          }
